...
const flipUv = (attribute: THREE.BufferAttribute | THREE.InterleavedBufferAttribute | undefined): void => {
	if (!attribute) {
		return;
	}
...
const populateScene = (scene: THREE.Scene, cb: (mesh: THREE.Object3D) => void): void => {
	...
	(async () => {
		...
		dracoLoader.setDecoderPath(path.resolve(import.meta.dirname, '../../../node_modules/three/examples/jsm/libs/draco/gltf/'));
		
		const loader = new GLTFLoader();
		loader.setDRACOLoader(dracoLoader as unknown as Parameters<typeof loader.setDRACOLoader>[0]);
		loader.load(path.resolve(import.meta.dirname, '../models/LittlestTokyo.glb'), (gltf: GLTF) => {
