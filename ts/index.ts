...
const initCore = (_opts: TInitOpts = {}): TCore3D => {
	...
	if (!isGles3) {
		...
		mutableWebgl.shaderSource = (shader, code) =>
			shaderSource(
				shader,
				code
					.replaceAll(/^\s*?(#version|precision).*?($|;)/gmu, '')
					.replace(/^/u, '#extension GL_ARB_shading_language_420pack : require\n')
					.replace(/^/u, '#extension GL_ARB_explicit_attrib_location : enable\n')
					.replace(/^/u, '#version 140\n')
					.replaceAll('gl_FragDepthEXT', 'gl_FragDepth')
					.replace('#extension GL_EXT_frag_depth : enable', '')
					.replaceAll(/\bhighp\s+/gu, ''),
			);
	}
